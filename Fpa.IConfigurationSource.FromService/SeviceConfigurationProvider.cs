    ...
    public class SeviceConfigurationProvider : ConfigurationProvider
    {
        ...
        public override void Load()
        {
            using (var client = new WebClient())
            {
                ...
                {
                    throw new JsonReaderException("Не удалось преобразовать результат ответа сервера конфигураций к Json объекту");
                }
                catch (Exception ex)
                {
                    throw new Exception("Не удалось преобразовать полученный результат к Key:Value");
                }
            }
        }
    }
